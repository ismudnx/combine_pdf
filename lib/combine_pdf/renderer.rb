  ...
  module Renderer
    ...
    def format_string_to_pdf(object)
      ...
      if object.length == 0 || obj_bytes.min <= 31 || obj_bytes.max >= 127 # || (obj_bytes[0] != 68  object.match(/[^D\:\d\+\-Z\']/))
        ...
        ('(' + ([].tap { |out| obj_bytes.each { |byte| out.concat(STRING_REPLACEMENT_ARRAY[byte]) } } ).pack('C*') + ')')
      end
    end
    ...
    def format_name_to_pdf(object)
      ...
      # from the :standard=>      # When writing a name in a PDF file, a SOLIDUS (2Fh) (/) shall be used to introduce a name. The SOLIDUS is not part of the name but is a prefix indicating that what follows is a sequence of characters representing the name in the PDF file and shall follow these :rules=>      # a) A NUMBER SIGN (23h) (#) in a name shall be written by using its 2-digit hexadecimal code (23), preceded by the NUMBER SIGN.
    ...
    def format_array_to_pdf(object)
      ...
      ('[' + (object.collect { |item| object_to_pdf(item) }).join(' ') + ']')
    end
    ...
    def format_hash_to_pdf(object)
      # if the object is only a :reference=>      # special conditions apply, and there is only the setting of the reference (if needed) and output
...
#########################################################
# PDF object types cross :reference=># Indirect objects, references, dictionaries and streams are Hash
# arrays are Array
# strings are String
